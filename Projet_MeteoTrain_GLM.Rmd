---
title: "Projet GLM-Choix de modèles Meteo"
...
## **Introduction** 
  ...
  Nous avons à disposition des données météorologiques dont nous allons nous servir pour pouvoir prédire si le lendemain sera un temps pluvieux ou sec à Bâle en Suisse.

Pour ce faire, nous allons dans un premier temps nous approprier les données, en effectuant divers traitements sur les données, pour en faciliter la lecture (pour les 2 jeux de données Meteotrain et Meteotest).

Dans cette première étape, nous allons observer les données par le summary fourni afin d'avoir un premier aperçu de celles-ci, tracer le corrélogramme, et regarder graphiquement ce que la variable cible peut nous révéler comme informations.


A travers différentes approches, nous allons mettre en oeuvre la régression logistique. 

En premier lieu, nous réaliserons l'approche par l'échantillon de validation. Pour ce faire, nous divisons la data set Meteotrain en 2, une data set pour l'apprentissage, et l'autre pour la validation.
...
Comme notre data set est de grande dimension, il a semblé pertinent de réaliser une analyse en composantes principales pour réduire sa taille, puis utiliser les axes factoriels retenus comme régresseurs. 
Ainsi, et avec les 2 méthodes déjà utilisées auparavant, nous obtiendrons d'autres modèles.
Afin d'évaluer leur qualité d'ajustement aux données et leur performance prédictive, chaque modèle sera testé et/ou validé au moyen de différents critères.

En deuxième lieu, nous verrons la mise en oeuvre de l'approche par validation croisée k folds, qui viendra challenger les modèles obtenus. 
...
```
  ...
  Dans un premier temps, nous souhaitons voir s'il existe certaines corrélations entre les variables qui nous intéresse, étant donné que nous avons un très grand nombre de variables (> 40 variables). Pour ce faire, nous allons tracer le corrélogramme.
...
ggplot(meteotrain) +
  ...
  scale_x_continuous(breaks=c(2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018), labels=c(2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018)) + 
...
2018 est quant à elle l'année avec le moins de pluie.
...
```{r VitesseVent_RafalesVent}

qplot(RafalesVent.moy,VitesseVent_10m.moy , data = meteotrain) +
  geom_smooth() +
  ggtitle("Vitesse du vent en fonction des rafales de vent en temps sec/pluvieux") +
  xlab("Rafales (km/h)") +
...
Par ailleurs, nos graphiques démontrent que la vitesse du vent croît de façon proportionnelle aux rafales de vent, et ce, qu'il fasse beau ou mauvais temps (sec ou pluvieux).



  *La Nébulosité moyenne et l'Ensoleillement par temps sans/avec pluie*

```{r Nebulositemoyenne_ensoleillement}
qplot(Nebulosite.moy.moy,Ensoleillement , data = meteotrain) +
  geom_smooth() +
  ggtitle("La couverture du soleil en fonction de la couverture nuageuse en temps sans/avec pluie") +
  xlab("Nébulosité") +
  ylab("Ensoleillement") +
  facet_wrap(~ pluie.demain)+
  theme_linedraw() +
...
tr.meteotrain = meteotrain[training,] # notre échantillon d'apprentissage a 948 lignes
test.meteotrain = meteotrain[!training,] # notre échantillon de validation a 232 lignes
```
...
lg.modeljour = glm(pluie.demain~. -Jour, family = binomial, data = tr.meteotrain)
anova(lg.modelsat,lg.modeljour,test = "LRT")

lg.modelmois = glm(pluie.demain~. -Jour-Mois, family = binomial, data = tr.meteotrain)
anova(lg.modelsat,lg.modelmois, test = "LRT")

lg.modelannee = glm(pluie.demain~. -Jour-Mois-Annee, family = binomial, data = tr.meteotrain)
anova(lg.modelsat,lg.modelannee, test = "LRT")

lg.modelX = glm(pluie.demain~. -Jour-Mois-Annee-X, family = binomial, data = tr.meteotrain)
anova(lg.modelsat,lg.modelX, test = "LRT")
```
  
  Tous les tests montrent que la p-value est supérieure à 0,05.  
Cela nous amène à toujours considérer que le modèle réduit est toujours préféré (l'hypothèse H0 de nullité de leurs coefficients n'est pas rejetée). Nous allons donc les exclure pour la suite dans la tr.meteotrain (base d'apprentissage).
...
# Spécifions de nouveau le modèle saturé lg.modelsat (qui sera la référence) et le modèle de départ lg.modelnull
lg.modelsat = glm(
...
library(leaps)
lg.modelbsubset <- regsubsets(pluie.demain ~., tr.meteotrain1, nbest=1, int=T,nvmax = 40)
...
points(which.min(lg.summary$rss), lg.summary$rss[which.min(lg.summary$rss)], col =" red " , cex =2 , pch =20)
...
plot(lg.summary$adjr2 , xlab =" Number of Variables " , ylab =" Adjusted RSq " , type ="l")
points(which.max(lg.summary$adjr2), lg.summary$adjr2[which.max(lg.summary$adjr2)], col =" red " , cex =2 , pch =20)
```
...
plot(lg.summary$cp , xlab =" Number of Variables " , ylab =" Cp " ,type ="l")
points(which.min(lg.summary$cp), lg.summary$cp[which.min(lg.summary$cp)], col =" red " , cex =2 , pch =20)
```
...
plot(lg.summary$bic , xlab =" Number of Variables " , ylab =" BIC " ,type ="l")
points(which.min(lg.summary$bic), lg.summary$bic[which.min(lg.summary$bic)], col =" red " , cex =2 , pch =20)
```
...
```    
  ...
  -Cp de Mallows : le meilleur modèle est à 16 variables.  
Celui-ci correspondrait à :
...
Nous ferons ensuite un tableau récapitulatif de tous les modèles de la best subset selection et ceux de Stepwise selection. 
...
```{r FWDAic_Selection}
library(MASS)
lg.fwdaic.model <- stepAIC(lg.modelnull,scope=list(lower=lg.modelnull,upper=lg.modelsat),data = tr.meteotrain1, direction = "forward")

coef(lg.fwdaic.model,which.min(summary(lg.fwdaic.model)$aic))
 
with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain1, nvmax=40, method="forward")),data.frame(rsq,adjr2,cp,rss,bic,outmat))
quartz()
```
...
```{r FWDAic_Modele}
lg.FWDAicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.max + Pression.min + Temperature.min + RafalesVent.max + Nebulosite.faible.max + Nebulosite.forte.max + Temperature.max + DirectionVent_900hpa.moy + TotalNebulosite.min + DirectionVent_80m.moy + Pression.moy + Pression.max + Nebulosite.moy.moy, family = binomial, data = tr.meteotrain1)
...
lg.bwdaic.model <- stepAIC(lg.modelsat,data = tr.meteotrain1, direction = "backward")

coef(lg.bwdaic.model,which.min(summary(lg.bwdaic.model)$aic))

with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain1, nvmax=40, method="backward")),data.frame(rsq,adjr2,cp,rss,bic,outmat))
quartz()
```
...
```{r BWDAic_Modele}
 lg.BWDAicModel <- glm(formula = pluie.demain ~ Pression.moy + Precipitations + Enneigement + Nebulosite.moy.moy + VitesseVent_80m.moy + DirectionVent_80m.moy + DirectionVent_900hpa.moy + Temperature.max + 
...
Le modèle lg.BWDAicModel retenu aboutit avec un AIC = 1033.7 et 17 variables.       
...
lg.stepwaic.model <- stepAIC(lg.modelnull,scope=list(upper=lg.modelsat),data = tr.meteotrain1, direction = "both")

coef(lg.stepwaic.model,which.min(summary(lg.stepwaic.model)$aic))

with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain1, nvmax=40, method="seqrep")
...
```{r STEPWAic_Modele}
lg.STEPWAicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.max + Pression.min + RafalesVent.max + Nebulosite.faible.max + Temperature.max + DirectionVent_900hpa.moy + TotalNebulosite.min + DirectionVent_80m.moy + Pression.moy + Pression.max + Nebulosite.moy.moy, family = binomial, 
data = tr.meteotrain1)

summary(lg.STEPWAicModel)
```

  Le modèle STEPWModel retenu aboutit à un modèle avec un AIC = 1043.9 et 11 variables.    
...
```{r FWDBic_FWDCp_Selections}

lg.fwd.model <- regsubsets(pluie.demain ~.,data=tr.meteotrain1, nvmax=40, method="forward")
...
lg.FWDBicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.moy +  Temperature.min + Pression.min + Nebulosite.moy.max +
...
lg.FWDCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy + Rayonnement_solR+
VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Temperature.min + Pression.max + Pression.min+  TotalNebulosite.min + Nebulosite.forte.max + Nebulosite.moy.max +
            Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, 
            data = tr.meteotrain1)
...
```{r BWDBic_BWDCp_Selections}
lg.bwd.model <- regsubsets(pluie.demain ~.,data=tr.meteotrain1, nvmax=40, method="backward")
...
lg.BWDBicModel <- glm(formula = pluie.demain ~ Pression.moy+
                        Nebulosite.moy.moy + DirectionVent_900hpa.moy+
                        Temperature.max + Pression.max + Pression.min+                         Nebulosite.moy.max +
...
lg.BWDCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy +
VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Pression.max + Pression.min+Nebulosite.forte.max + Nebulosite.moy.max + Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, 
            data = tr.meteotrain1)
...
```{r STEPWBic_STEPWCp_Selections}
lg.stepw.model <- regsubsets(pluie.demain ~.,data=tr.meteotrain1, nbest = 1,nvmax = 40, method ="seqrep")
...
lg.STEPWCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy+
VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Pression.max + Pression.min + Nebulosite.forte.max + Nebulosite.moy.max +Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, 
            data = tr.meteotrain1)
...
#### **II.2.5 Tableau récapitulatif des modèles**

```{r Recap_Modeles1}
crit = c("Bic","Cp","R2 Ajusté","RSS",rep(c("Aic","Bic","Cp"),3),"Aic","Bic")
nbvar = c(6,16,18,22,
                13,5,19,
                17,8,16,
                11,6,16,4,3)
valAIC = c(1057.1,1034,1036.9,1034.5,
                 ...
                 1033.7,1050.7,1034,
                 1043.9,1057.1,1034,1065.4,1067.3)
valBIC = c(-230,-204.5,-172.3,-194.5,
                 -208.2,-226.5,-188.3,
                 -199.3,-226.3,-204.5,
                 -217.9,-230.2,-204.5,-228.7,-231.2)
...
valRSS = c(176.7,168.9,167.3,168.2,
                 171.9,178.7,168.1,
                 168.6,174.9,168.9,
                 172.6,176.7,168.9,179.5,180.4)

tablo=cbind(crit,nbvar,valAIC,valBIC,valCp,valR2ADJ,valRSS)
...
tabl.res %>%
mutate(Methode=c(rep("Exhaustive",4), rep("Forward",3), rep("Backward",3), rep("Stepwise",3),"Backward ACP","Exhaustive ACP")) %>%
...
#### **II.2.6 Approche par Analyse en Composantes Principales**
...
```{r ACP_Valeurs_Propres_Selection}
library(factoextra)
library(FactoMineR)
library(ggplot2)
library(grid)
...
acp.eig.val # critère de Kaiser : observer les valeurs propres supérieures à 1 #
...
En n'affichant que les variables de qualité de représentation cos2>=0.5, nous constatons pour l'effet taille sur l'axe 1,que les variables qui sont toutes corrélées positivement (du même côté de l'axe 1) sont :     
  - les variables qui se rapportent au VitesseVent/RafalesVent/DirectionVent (sauf DirectionVent_10m.moy, DirectionVent_900hpa.moy)  
  - les 3 variables relatives à la  Nebulosite qui sont TotalNebulosite.moy, Nebulosite.moy.moy et Nebulosite.faible.moy  


  Pour l'effet forme sur cet axe 1, il semble s'opposer le groupe des variables des caractéristiques VitesseVent/Rafales/DirectionVent citées ci-dessus à la variable Ensoleillement, si nous ne retenons que la qualité de représentation supérieure à 0,5.  
...
Pour l'effet forme, il y a opposition du groupe des variables liées à la Nebulosite au groupe des variables constitué par les variables VitesseVent/RafalesVent/DirectionVent et Ensoleillement.  

Ceci étant dit, la variable Ensoleillement est corrélée négativement au groupe des variables VitesseVent/RafalesVent/DirectionVent. Aussi, les 3 variables de Nebulosite sont corrélés négativement avec Ensoleillement. Ceci semble tout à fait logique car dès que le ciel se couvre de nuages, la durée d'ensoleillement s'écourte au cours de la journée.   
...
Avec l'analyse des variables faite plus haut, nous en déduisons que ces journées sont donc caratérisées par l'importance des nuages (pour le quart haut droite) et celles du vent de toutes natures (pour le quart bas droite) 
...
Dans le quart haut droite, nous avons également des points remarquables comme les observations : 693, 884, 402 ; ceux-ci figurent également parmi les plus contributeurs de l'axe 1.
...
```
  ...
  L'axe 1 :
Les variables qui contribuent le pus fortement sont toutes les variables VitesseVent/RafalesVent/DirectionVent déjà vues plus haut.  
...
===> cet axe 1 représenterait donc les journées venteuses (avec vitesse, rafales et direction importantes).
...
De plus, les variables Pression sont aussi de grandes contributrices à cet axe, avoisinant une contribution à 8, mais la qualité de représentation à 0,3 est assez peu importante. On pourrait cependant les conserver mais nous les reverrons sur un autre plan. De même que le rayonnement solaire, qui apporte sa contribution aussi (plus de 6) à cet axe avec un cos2 moyen à 0.3, mais présente une bonne corrélation avec cette dimension 3.  
...
Les journées de temps sec sont plus situées dans le quart bas gauche en relation avec la pression, et aussi dans le quart haut gauche proche de l'axe 1.
...
Pour résumer, nous voyons à travers le graphe ci-dessus axe par axe, les variables qui obtiennent les coefficients de corrélation les plus significatifs (positif comme négatif).  
En complément de notre analyse, nous avons les déductions qui suivent :  
...
# Récupération des individus et variables projetés sur les axes factoriels #
...
# Standardisation de la base test.meteotrain sans les variables supplémentaires de l'acp #
test.meteotrain.std <- as.data.frame(scale(test.meteotrain[,-c(1:4,45)], center = TRUE,scale = TRUE))
...
acp.test.proj <- matrix (rep(0, nrow(test.meteotrain.std)*ncol(acp.res$ind$coord)),
                         nrow(test.meteotrain.std), ncol(acp.res$ind$coord)) 
for (i in 1:8){ 
...
Nous allons opter pour une méthode pas à pas Forward, Backward et Stepwise pour construire 3 modèles, avec le critère AIC.  
...
# Spécifions de nouveau le modèle saturé lg.modelsat.acp (qui seront les références) et le modèle de départ lg.modelnull.acp
lg.modelsat.acp = glm(
...
lg.fwdaic.model.acp <- stepAIC(lg.modelnull.acp,scope=list(upper=lg.modelsat.acp),data = tr.meteotrain_proj, direction = "forward")

coef(lg.fwdaic.model.acp,which.min(summary(lg.fwdaic.model.acp)$aic))

with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain_proj, nvmax=8,nbest = 2, method="forward")
),data.frame(rsq,adjr2,cp,rss,bic,outmat))
quartz()
```
...
```{r FWDAic_Model_acp}
lg.FWDAicModel_acp <- glm(formula = pluie.demain ~Dim.3 + Dim.1 + Dim.2 + Dim.5 , family = binomial, data = tr.meteotrain_proj)
...
lg.bwdaic.model.acp <- stepAIC(lg.modelsat.acp,data = tr.meteotrain_proj, direction = "backward")

coef(lg.bwdaic.model.acp,which.min(summary(lg.bwdaic.model.acp)$aic))

with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain_proj, nvmax=8,nbest = 2, method="backward")),data.frame(rsq,adjr2,cp,rss,bic,outmat))
quartz()
```
...
```{r BWDAic_Model_acp}
lg.BWDAicModel_acp <- glm(formula = pluie.demain ~Dim.3 + Dim.1 + Dim.2 + Dim.5 , family = binomial, data = tr.meteotrain_proj)
...
lg.stepwaic.model.acp <- stepAIC(lg.modelnull.acp,scope=list(upper=lg.modelsat.acp),data = tr.meteotrain_proj, direction = "both")

coef(lg.stepwaic.model.acp,which.min(summary(lg.stepwaic.model.acp)$aic))

with(summary(regsubsets(pluie.demain ~.,data=tr.meteotrain_proj, nvmax=8,nbest = 2, method="seqrep")
),data.frame(rsq,adjr2,cp,rss,bic,outmat))
quartz()
```
...
lg.STEPWAicModel_acp <- glm(formula = pluie.demain ~ Dim.3 + Dim.1 + Dim.2 + Dim.5, family = binomial, 
...
```{r Modele_regsubset_acp}
# Méthode Best subset selection

lg.modelbsubset_acp <- regsubsets(pluie.demain ~., tr.meteotrain_proj, nbest=2, int=T,nvmax = 8)
...
```{r BSSBicModele_acp}
lg.BSSBicModel_acp <- glm(formula = pluie.demain ~ Dim.1+Dim.2+Dim.3, family = binomial, 
            data = tr.meteotrain_proj)
...
lg.BSSRSSModel <- glm(pluie.demain~Pression.moy+Precipitations+Enneigement+Nebulosite.moy.moy+Ensoleillement+VitesseVent_80m.moy+DirectionVent_80m.moy+DirectionVent_900hpa.moy+Temperature.max+Pression.max+Pression.min+TotalNebulosite.min+Nebulosite.forte.max+Nebulosite.moy.max+Nebulosite.faible.max+VitesseVent_10m.max+VitesseVent_10m.min+RafalesVent.max,family = binomial,data = tr.meteotrain)

lg.BSSR2ADJModel <- glm(pluie.demain~Pression.moy+Precipitations+Enneigement+
Nebulosite.moy.moy+Ensoleillement+VitesseVent_10m.moy+VitesseVent_80m.moy+
DirectionVent_80m.moy+DirectionVent_900hpa.moy+RafalesVent.moy+Temperature.max+
Temperature.min+Humidite.max+Pression.max+Pression.min+TotalNebulosite.min+
Nebulosite.forte.max+Nebulosite.moy.max+Nebulosite.faible.max+VitesseVent_10m.max+
VitesseVent_10m.min+VitesseVent_900hpa.max,family = binomial,data = tr.meteotrain)

lg.BSSCpModel <- glm(pluie.demain~Pression.moy+Precipitations+Enneigement+Nebulosite.moy.moy+VitesseVent_80m.moy+DirectionVent_80m.moy+DirectionVent_900hpa.moy+
Temperature.max+Pression.max+Pression.min+Nebulosite.forte.max+Nebulosite.moy.max+
Nebulosite.faible.max+VitesseVent_10m.max+VitesseVent_10m.min+RafalesVent.max,
family = binomial,data = tr.meteotrain)

lg.BSSBicModel <- glm(pluie.demain~Nebulosite.moy.moy+Temperature.max+Pression.min+
Nebulosite.moy.max+Nebulosite.faible.max+RafalesVent.max,family = binomial,
data = tr.meteotrain)

lg.FWDAicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.max + Pression.min + Temperature.min + RafalesVent.max + Nebulosite.faible.max + Nebulosite.forte.max + Temperature.max + DirectionVent_900hpa.moy + TotalNebulosite.min + DirectionVent_80m.moy + Pression.moy + Pression.max + Nebulosite.moy.moy, family = binomial, data = tr.meteotrain)
...
lg.STEPWAicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.max + Pression.min + RafalesVent.max + Nebulosite.faible.max + Temperature.max + DirectionVent_900hpa.moy + TotalNebulosite.min + DirectionVent_80m.moy + Pression.moy + Pression.max + Nebulosite.moy.moy, family = binomial, data = tr.meteotrain)

lg.FWDBicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.moy +  Temperature.min + Pression.min + Nebulosite.moy.max +RafalesVent.max, family = binomial, 
data = tr.meteotrain)

lg.FWDCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy + Rayonnement_solR+VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Temperature.min + Pression.max + Pression.min+  TotalNebulosite.min + Nebulosite.forte.max + Nebulosite.moy.max +Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, data = tr.meteotrain)

lg.BWDBicModel <- glm(formula = pluie.demain ~ Pression.moy+Nebulosite.moy.moy + DirectionVent_900hpa.moy+Temperature.max + Pression.max + Pression.min+            Nebulosite.moy.max +VitesseVent_10m.max, family = binomial, data = tr.meteotrain)

lg.BWDCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy +VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Pression.max +
Pression.min+Nebulosite.forte.max + Nebulosite.moy.max + Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, 
data = tr.meteotrain)

lg.STEPWBicModel <- glm(formula = pluie.demain ~ Nebulosite.moy.moy +
Temperature.max + Pression.min+Nebulosite.moy.max +Nebulosite.faible.max+
RafalesVent.max, family = binomial, data = tr.meteotrain)

lg.STEPWCpModel <- glm(formula = pluie.demain ~ Pression.moy+ Precipitations+ Enneigement+ Nebulosite.moy.moy+VitesseVent_80m.moy+ DirectionVent_80m.moy+ DirectionVent_900hpa.moy+ Temperature.max+ Pression.max + Pression.min + Nebulosite.forte.max + Nebulosite.moy.max +Nebulosite.faible.max + VitesseVent_10m.max+ VitesseVent_10m.min + RafalesVent.max, family = binomial, 
data = tr.meteotrain)

lg.BWDAicModel_acp <- glm(formula = pluie.demain ~Dim.3 + Dim.1 + Dim.2 + Dim.5 , family = binomial, data = tr.meteotrain_proj)

lg.BSSBicModel_acp <- glm(formula = pluie.demain ~ Dim.1+Dim.2+Dim.3, family = binomial, data = tr.meteotrain_proj)

lg.modelsat_acp <- glm(formula = pluie.demain ~Dim.1 + Dim.2 +Dim.3 + Dim.4+ Dim.5+
Dim.6+Dim.7+Dim.8, family = binomial, data = tr.meteotrain_proj)
```
...
### **II.4 Evaluation et validation des modèles (performance prédictive des modèles)**
...
Nous allons passer au crible les 16 modèles retenus en passant à la loupe leur capacité prédictive. Ainsi, nous allons calculer les erreurs de prédiction de plusieurs façons : l'erreur absolue, le RMSEP (root mean squared error of prediction) et le MSEP (mean squared error of prediction) et voir quel est le modèle qui se démarque avec l'erreur minimum.
...
```{r ABSError_MSEP_RMSEP_Modeles, results='hold'}
...
# Calculs des erreurs de prédiction absolues #

mean(abs(pred.BSSRSSmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BSSR2ADJmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BSSCpmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BSSBicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.FWDAicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BWDAicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.STEPWAicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.FWDBicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.FWDCpmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BWDBicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BWDCpmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.STEPWBicmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.STEPWCpmodel - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BWDAicmodel_acp - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.BSSBicmodel_acp - test.meteotrain$pluie.demain), na.rm = T)
mean(abs(pred.modelsat_acp - test.meteotrain$pluie.demain), na.rm = T)


# Calculs des RMSEP #

sqrt(mean((pred.BSSRSSmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BSSR2ADJmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BSSCpmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BSSBicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.FWDAicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BWDAicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.STEPWAicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.FWDBicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.FWDCpmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BWDBicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BWDCpmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.STEPWBicmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.STEPWCpmodel - test.meteotrain$pluie.demain)^2))
sqrt(mean((pred.BWDAicmodel_acp - test.meteotrain_proj$pluie.demain)^2))
sqrt(mean((pred.BSSBicmodel_acp - test.meteotrain_proj$pluie.demain)^2))
sqrt(mean((pred.modelsat_acp - test.meteotrain_proj$pluie.demain)^2))

# Calculs des MSEP #

mean((pred.BSSRSSmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BSSR2ADJmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BSSCpmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BSSBicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.FWDAicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BWDAicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.STEPWAicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.FWDBicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.FWDCpmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BWDBicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BWDCpmodel - test.meteotrain$pluie.demain)^2)
mean((pred.STEPWBicmodel - test.meteotrain$pluie.demain)^2)
mean((pred.STEPWCpmodel - test.meteotrain$pluie.demain)^2)
mean((pred.BWDAicmodel_acp - test.meteotrain_proj$pluie.demain)^2)
mean((pred.BSSBicmodel_acp - test.meteotrain_proj$pluie.demain)^2)
mean((pred.modelsat_acp - test.meteotrain_proj$pluie.demain)^2)
```

D'après le critère de l'erreur absolue, le modèle qui semble avoir le meilleur score ( c-à-d le min) est lg.BSSBicmodel_acp, avec un score à 1.016188.  
...
Rappelons que le modèle lg.BSSBicmodel_acp issu de l'ACP, a  donc les 3 premières composantes principales dans sa construction. (Dim.1, Dim.2 et Dim.3)  
 ...
 Pour challenger cette décision, nous souhaitons évaluer d'une autre façon la qualité prédictive de nos 3 modèles. Il existe en effet d'autres moyens.  
...
C'est à l'issue de cette étape que nous déciderons du modèle à retenir : car c'est bien par sa qualité prédictive qu'un modèle est déterminément choisi parmi plusieurs modèles.  
...
test.meteotrain_proj$pluie.demain=as.factor(ifelse(test.meteotrain_proj$pluie.demain==1,0,1))

test.meteotrain$pluie.demain=as.factor(ifelse(test.meteotrain$pluie.demain==1,0,1))

confusionMatrix(data=prd.BSSRSSmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BSSR2ADJmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BSSCpmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BSSBicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.FWDAicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BWDAicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.STEPWAicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.FWDBicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.FWDCpmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BWDBicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BWDCpmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.STEPWBicmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.STEPWCpmodel, reference = test.meteotrain$pluie.demain)

confusionMatrix(data=prd.BWDAicmodel_acp, reference = test.meteotrain_proj$pluie.demain)

confusionMatrix(data=prd.BSSBicmodel_acp, reference = test.meteotrain_proj$pluie.demain)

confusionMatrix(data=prd.modelsat_acp, reference = test.meteotrain_proj$pluie.demain)
```
...
Nous allons passer au crible tous les modèles et évaluer leurs courbes ROC et calculer les Auc respectifs.  


  *Courbe ROC et Auc* 
    
```{r Courbe_ROC, fig.height=5, fig.width=7, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r Ajustement_modele_Diagramme_fiabilite, fig.height=5, fig.width=7, message=FALSE, warning=FALSE, echo=FALSE}
...
prop.pos <- apply(table(scor.FWDAicmodel,test.meteotrain$pluie.demain),1,function(x){x[2]/sum(x)})
...
# Calcul de la moyenne sur chaque groupe #
moy.scor.STEPWAicmodel <- tapply(sort(pred.STEPWAicmodel), scor.STEPWAicmodel, mean)
print(moy.scor.STEPWAicmodel)

# Proportion des positifs par groupe #
test.meteotrain$pluie.demain=as.factor(ifelse(test.meteotrain$pluie.demain==1,0,1))
table(scor.STEPWAicmodel,test.meteotrain$pluie.demain)
prop.pos <- apply(table(scor.STEPWAicmodel,test.meteotrain$pluie.demain),1,function(x){x[2]/sum(x)})
...
test.meteotrain$pluie.demain=as.factor(ifelse(test.meteotrain$pluie.demain==1,0,1))
table(scor.FWDAicmodel,test.meteotrain$pluie.demain)
prop.pos <- apply(table(scor.FWDBicmodel,test.meteotrain$pluie.demain),1,function(x){x[2]/sum(x)})
...
prop.pos <- apply(table(scor.BWDBicmodel,test.meteotrain$pluie.demain),1,function(x){x[2]/sum(x)})
...
# Tri décroissant et découpage des scores en groupes #
scor.BWDAicmodel_acp <- cut(sort(pred.BWDAicmodel_acp),breaks=seq(from=0.0, to=1.0, by=0.25),include.lowest = TRUE)
table(scor.BWDAicmodel_acp)

# Calcul de la moyenne sur chaque groupe #
moy.scor.BWDAicmodel_acp <- tapply(sort(pred.BWDAicmodel_acp), scor.BWDAicmodel_acp, mean)
print(moy.scor.BWDAicmodel_acp)

# Proportion des positifs par groupe #
test.meteotrain_proj$pluie.demain=as.factor(ifelse(test.meteotrain_proj$pluie.demain==1,0,1))
table(scor.BWDAicmodel_acp,test.meteotrain_proj$pluie.demain)
prop.pos <- apply(table(scor.BWDAicmodel_acp,test.meteotrain_proj$pluie.demain),1,function(x){x[2]/sum(x)})

# Graphe #
plot(moy.scor.BWDAicmodel_acp, prop.pos, main="Diagramme de fiabilité", type="b", xlim=c(0,1), ylim=c(0,1))
abline(a=0,b=1)


# Modèle lg.BSSBicmodel_acp #
# Tri décroissant et découpage des scores en groupes #
scor.BSSBicmodel_acp <- cut(sort(pred.BSSBicmodel_acp),breaks=seq(from=0.0, to=1.0, by=0.25),include.lowest = TRUE)
table(scor.BSSBicmodel_acp)

# Calcul de la moyenne sur chaque groupe #
moy.scor.BSSBicmodel_acp <- tapply(sort(pred.BSSBicmodel_acp), scor.BSSBicmodel_acp, mean)
print(moy.scor.BSSBicmodel_acp)

# Proportion des positifs par groupe #
test.meteotrain_proj$pluie.demain=as.factor(ifelse(test.meteotrain_proj$pluie.demain==1,0,1))
table(scor.BSSBicmodel_acp,test.meteotrain_proj$pluie.demain)
prop.pos <- apply(table(scor.BSSBicmodel_acp,test.meteotrain_proj$pluie.demain),1,function(x){x[2]/sum(x)})
...
prop.pos <- apply(table(scor.modelsat_acp,test.meteotrain_proj$pluie.demain),1,function(x){x[2]/sum(x)})
...
Essayons le test de Hosmer et Lemeshow.  



  *Test de Hosmer et Lemeshow*
    
```{r Ajustement_test_Hosmer_Lemeshow, results='hold'}
...
Nous décidons au vu de ces différentes méthodes de retenir le modèle issu de l'acp lg.BSSBicModel_acp car c'est celui qui regroupe le plus de critères satisfaisants voire très satisfaisants : l'Erreur absolue, le MSE et le RMSEP au minimum, une matrice de confusion avec une très bonne précision (Accuracy = 0.72), une courbe ROC avec un Auc = 0.79, et un test de Homer et Lemeshow des plus satisfaisants (p-value = 0.72).    
...
barplot(contrib.deviance, horiz=T,las=2,main="Contribution Deviance",cex.names=0.5)
...
plot(influence(lg.BSSBicModel_acp)$hat,type="h",ylab="hii", main = "Points leviers", cex.main = 0.9, cex.lab=0.7)
seuil1 = 3*p/n
...
plot(dcook,type="h",ylab="Distance de Cook",las=2,main = "Distance de Cook",cex.lab=0.7, cex.main=0.9)
```
...
```{r Graphe_residus, fig.height=5, fig.width=7, message=FALSE, warning=FALSE, echo=FALSE}
library(car)
...
En théorie, nous devrions avoir 95% des résidus se trouvant entre -2 et 2, ici, ils le sont bien, seuls une dizaine de poins sont à l'extérieur de l'intervalle (certainement les points aberrants déjà évoqués entre autres).  
...
Nous allons utiliser la base tr.meteotrain_proj issue de l'Acp, qui sera donc divisée en 10 blocs, puis nous validerons avec la test.meteotrain_proj.
...
Pour rappel, cette méthode consiste à ajuster le modèle sur les n-1 observations (journées ici), et à effectuer la prédiction sur l'observation mise de côté. Nous répétons ensuite cette procédure n fois. Aussitôt cette opération faite, chaque obsevation aura été prédite une et une seule fois.  
...
for ( i in 1:nrow(meteotrain)) {
  # modèle
    lg.LOOCVmodel = glm(pluie.demain~.,data = meteotrain[-i,],family = binomial)
  # prédiction
...
meteotest.std <- as.data.frame(scale(meteotest[,-c(1:4)], center = TRUE,scale = TRUE))

#Projection de la matrice test.meteotrain sur les axes factoriels de ACP#
pdtmat = as.matrix(meteotest.std) %*% as.matrix(acp.res.var.coord)

meteotest.proj <- matrix (rep(0, nrow(meteotest.std)*ncol(acp.res$ind$coord)),
                         nrow(meteotest.std), ncol(acp.res$ind$coord)) 
for (i in 1:8){ 
...
pred.meteotest = predict(lg.BSSBicModel_acp,new.meteotest_proj, type = "response")
...
Pour conclure, nous avons mené à bien notre mission de prédire la pluie à partir de meteotest. Rappelons que nous avons effectué selon 3 approches la validation : approche par l'échantillon de validation (80/20 train-test sur la meteotrain), approche par K-fold cross validation et l'approche par la Leave-One-Out cross validation.  

Avec la 1ère approche (par échantillon de validation), nous avions dû procéder à la division de la base Meteotrain en 2 (80/20) et mettre en oeuvre les recherches à partir de la base d'apprentissage de celle-ci. (c'est-à-dire sur les 80% de la meteotrain).   
Au départ nous avions plus de 40 variables, nous avons opté en premier lieu pour la construction de modèles sur la totalité de ces variables (avec un premier modèle saturé en guise de référence) et malgré la complexité de la tâche et la lourdeur des calculs parfois, nous avions obtenu 9 modèles assez satsifaisants.  

Dans un deuxième temps, la très grande taille de la base nous a amené à réfléchir sur la difficulté à la traiter au mieux.   
C'est pourquoi il nous a paru judicieux de procéder à une analyse en composantes principales (ACP) qui se prétait bien à cette étude car elle permet de réduire au mieux la dimension de la base tout en gardant le maximum d'informations. Ainsi, nous avons réussi à construire 3 autres modèles avec les composantes principales comme régresseurs cette fois-ci.  
...
Le modèle que nous avons alors décider de retenir est le modèle issu de l'Acp que nous avons obtenu par une méthode de recherche exhaustive Best subset selection avec le critère Bic (minimum). Celui-ci réunissait les meilleures conditions en terme de performance prédictive lorsque nous l'avons soumis aux tests d'ajustement et de validation avec les autres modèles.  

En seconde et troisième vue, nous avons essayé les 2 approches par K-fold cross validation et Leave_one_out cross validation, (les 2 cette fois ci étant basées sur la meteotrain dans son intégralité), mais cela n'était pas plus satisfaisant comparé à l'approche par l'échantillon de validation.  

Enfin, nous avons visualisé les résidus sans pour autant approfondir les résultats, faute de moyens pour expliciter les détails.  

Le fichier des résultats de prédiction la Meteotest se trouve dans : 
